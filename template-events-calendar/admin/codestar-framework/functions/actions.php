...
if ( ! function_exists( 'csf_reset_ajax' ) ) {
  function csf_reset_ajax() {
      ...
      if ( $_SERVER['REQUEST_METHOD'] !== 'POST' ) {
          wp_send_json_error( array( 'error' => esc_html__( 'Invalid request method.', 'csf' ) ) );
      }

      if ( ! current_user_can( 'manage_options' ) ) {
          wp_send_json_error( array( 'error' => esc_html__( 'You do not have permission to perform this action.', 'csf' ) ) );
      }

      $nonce  = ! empty( $_POST['nonce'] ) ? sanitize_text_field( wp_unslash( $_POST['nonce'] ) ) : '';
      $unique = ! empty( $_POST['unique'] ) ? sanitize_key( wp_unslash( $_POST['unique'] ) ) : '';
      ...
      if ( empty( $unique ) ) {
          wp_send_json_error( array( 'error' => esc_html__( 'Error: Invalid option key.', 'csf' ) ) );
      }
